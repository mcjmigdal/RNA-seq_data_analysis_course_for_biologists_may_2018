---
title: 'RNA Club meets NGSchool: Tutorial of RNA-seq Data Analysis for Biologists'
...
packages_bioc <- c("BiocInstaller", "BiocParallel", "SummarizedExperiment", "GenomicAlignments", "BiocGenerics", "DESeq2", "Rsamtools", "GO.db", "biomaRt", "GenomicFeatures", "DO.db","GenomicRanges", "clusterProfiler")
packages_cran <- c("reshape2","plyr","tidyr","tibble", "dplyr", "data.table", "RColorBrewer", "ggplot2")

check.packages.bioc(packages_bioc)
check.packages.cran(packages_cran)
```
...
files  <- list.files("./RNA-seq_data_analysis_course_for_biologists_may_2018/quants",
...
**Heatmap of genes with the highest expression**
...
#draw the heatmap
assay(rld)[,c(1,4,7,2,5,8,3,6,9)][select,]
pheatmap(assay(rld)[,c(1,4,7,2,5,8,3,6,9)][select,], cluster_rows=FALSE, show_rownames=TRUE,
         cluster_cols=FALSE)
```
...
Res_30_48 = results(dds, contrast = c('time', '48hpf', '30hpf'), independentFiltering=T)
...
Res_48_72 = results(dds, contrast = c('time', '72hpf', '48hpf'), independentFiltering=T)
Res_48_72
...
mart <- biomaRt::useDataset("drerio_gene_ensembl", useMart("ensembl"))
#check what are available attributes stored in mart object
#biomaRt::listAttributes(mart=mart)
#getting external gene name and entrez gene name
biomaRt_list <- getBM( filters = "ensembl_gene_id", attributes = c("ensembl_gene_id", "external_gene_name", "entrezgene"), values=DEG_cutoff$`Gene stable ID`, mart= mart, bmHeader = TRUE, uniqueRows = TRUE)
#merging novel gene names with DEG table
DEG_annot = dplyr::inner_join(biomaRt_list , DEG_cutoff, by="Gene stable ID" )
#saving data as csv
